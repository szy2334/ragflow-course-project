...
<template>
  <section class="page reports-page"><PageHeader eyebrow="沉淀研究结果" title="阅读报告" description="将已核验的结论、证据和局限整理为可导出的阅读报告。"><button class="primary-button" @click="showCreate = true"><Plus :size="18" />新建报告</button></PageHeader><div v-if="reports.length" class="report-grid"><article v-for="report in reports" :key="report.report_id" class="report-card" @click="router.push(`/reports/${report.report_id}`)"><div class="report-icon"><FileText :size="23" /></div><div><StatusPill :status="report.status" /><h2>{{ report.title }}</h2><p>{{ report.paper_ids.length }} 篇论文 · {{ report.claims.length }} 条已核验结论</p><small>创建于 {{ formatChinaDate(report.created_at) }}</small></div><FileOutput :size="18" /></article></div><div v-else class="empty-state"><FileOutput :size="35" /><h2>创建第一份阅读报告</h2><p>该页只读取当前浏览器已创建或打开过的报告详情，避免调用设计文档未定义的列表接口。</p><button class="primary-button" @click="showCreate = true"><Plus :size="18" />新建阅读报告</button></div><p v-if="error" class="inline-error">{{ error }}</p></section>
