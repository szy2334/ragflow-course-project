...
const sourceName = computed(() => isStandard.value ? evidence.standard_name ?? evidence.paper_title : evidence.paper_title)
const sourceLocation = computed(() => isStandard.value ? `标准 ${evidence.standard_version ?? '未标注版本'}` : `${evidence.section_title} · 第 ${evidence.page_number} 页`)
</script>
