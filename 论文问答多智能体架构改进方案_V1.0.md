...
所有调用 GPT-5.5 的智能体（`IntentRouterAgent`、`PaperUnderstandingAgent`、`AnswerGeneratorAgent`）均通过 **AgentRunner** 统一发起受控的 LLM 请求，而非各自直接持有 `StructuredLlm` 实例。AgentRunner 负责单次模型调用的机械生命周期，不做业务决策：封装 Prompt 渲染、通过 HTTP 客户端调用模型、Schema 校验、失败重试和 Metrics 收集；返回 `tuple[TModel, ModelCallMetrics]`（原始结构化输出 + 调用指标）。模型配置由 Harness 提供。AgentRunner 与 Harness 的分工详见第 8 节。

**AgentRunner 不做的：** 不将模型输出转换为 `AgentResult`、不提取 `Claim`、不计算置信度、不拼接证据列表。这些语义处理是每个智能体自己的职责——`PaperUnderstandingAgent` 将 `PaperUnderstanding` 的 `facts[]` 转为 `Claim` 列表，`ReviewAgent` 将 `ReviewOpinions` 的二维结构展平，每种智能体的转换逻辑完全不同。AgentRunner 管"调得对不对"，智能体管"结果是什么意思"。

**可行性确认：** AgentRunner 在论文阅读模块中的引入是低风险、高收益的。现有四个智能体（`ControllerAgent`、`PaperUnderstandingAgent`、`ReviewAgentA`、`ReviewAgentB`）各自重复 Prompt 渲染 → LLM 调用 → 异常处理的三步模式。改进方案将其替换为三个调用 GPT 的智能体（`IntentRouterAgent`、`PaperUnderstandingAgent` 改为仅做总结、`AnswerGeneratorAgent`），每次问答的 LLM 调用从 3~5 次降至 2 次。AgentRunner 将所有这些智能体的重试策略、Schema 校验、Metrics 收集和流式输出收拢到一处，且不增加任何 LLM 调用次数。格式审查模块的四个新智能体（RAGFlow 检索、规范反思、格式校验、校验反思）同样收益于此——它们通过 AgentRunner 发起请求，与论文阅读模块共享同一份调用生命周期管理代码。
...
两个主控均不调用 GPT，共享 AgentRunner（LLM 调用门面）、Harness 层和任务生命周期。
