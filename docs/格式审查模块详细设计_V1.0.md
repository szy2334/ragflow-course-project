...
格式审查模块面向投稿前的论文格式自检。用户上传 PDF，并选择目标投稿场所、规范版本和投稿模式。系统从 RAGFlow 中该投稿场所和规范版本对应的数据集内，只检索通用规则文档与用户所选投稿模式规则文档中的格式规则（以结构化 Chunk 形式存储的规则条文，每条含章节来源、检索关键词和清洗后的规则原文）。论文按语义章节与跨章节版面单元拆分为受控审查块；每个块在有限上下文中完成格式判断与证据核验，随后由汇总智能体整合已核验发现，输出按类别组织的、带规范依据、页码、坐标和修改建议的最终报告。模块不做逐规则孤立校验，也不再将整篇论文一次性送入模型。
...
纳入本期的审查范围覆盖论文的整体格式与版面：页面尺寸与页边距、标题与正文的字体字号层级、图表的编号与位置、参考文献的格式、以及投稿场所格式规范中明确规定的其他可观察版面要求。系统保留完整的投稿场所规则清单，但以语义审查块组织模型输入；跨章节或全篇规则由专门的跨章节块和最终汇总阶段处理。
...
| 上下文装配 | 为每个审查块冻结输入 token 预算和选择策略：纳入适用于该块的完整规范规则、该块及必要跨页关联的解析事实和双证据锚点，禁止将整篇 PDF 或不相关投稿场所格式规范无差别塞入上下文。规则不得截断；事实选择必须记录纳入和未纳入的 ID、选择理由与预算。块仍无法完整装配时标记为 `unverifiable`。 |
...
规范源 JSONL 的每条规则必须带有 `venue_id`、`format_version`、`submission_mode`、`target_document`、`canonical_rule_id`、`rule_category`、`source_document_id`、`section_path`、`effective_from`、`status`。其中前两项是数据集范围不变量；`submission_mode` 取 `shared` 或一个具体投稿模式，`target_document` 指出规则导入的共享或模式专用文档；`canonical_rule_id` 用于把多个规则片段还原为同一条规范规则。V1.1 还要求系统侧版本化规则清单记录 `applicable_unit_kinds`、`is_global`、`requires_cross_unit`、`applicability_conditions` 与 `evidence_selector`，其字段定义和迁移策略以第 17.2.1 节为准。当前 RAGFlow Chunk 接口不保存任意自定义 Chunk 元数据，因此这些字段须同时写入系统侧版本化规则清单；RAGFlow 文档只保存 `venue_id`、`format_version`、`submission_mode` 和 `rule_scope` 等文档级元数据。规范入库时必须校验规则清单，并在审查快照中冻结该清单版本。检索适配器仅查询当前档案的 `dataset_id`，以冻结的共享文档 ID 和所选模式文档 ID 作为硬过滤条件；它依据返回的 `document_id` 和系统侧规则清单校验范围与完整性。

### 3.2 块级综合格式检查

系统不做逐规则孤立判定。V1.1 中，`retrieve_standard` 先完成档案级检索和确定性完整性检查，`plan_review_units` 并行产生语义块；两者汇合后按第 17.2.2 节的结构化范围规则确定性分发。每个块获得完整的预期规则集合、局部 PDF 版面事实和跨块所需的最小关联事实。`SectionFormatCheckAgent` 对一个块输出按类别组织的候选发现；`SectionFormatValidationAgent` 对该块进行证据反思与门禁。只有块级反思通过的发现才能进入流式结果和最终汇总。

块级综合检查同时保留以下能力：

- **规则完整：** 某块的适用规则作为完整集合进入上下文，系统侧规则清单仍证明召回完整性。
- **局部关联：** 标题、正文、图题等相互关联元素处于同一块或由显式跨块事实关联，不按单条规则割裂判断。
- **跨块一致性：** 页数、页边距、匿名信息、引用/参考文献一致性等全篇规则进入跨章节块；最终汇总智能体仅整合已核验的结构化结果，不重新读取整篇 PDF。
- **可流式展示：** 块级结果在反思通过后即可回传前端，用户无需等待全文模型调用结束。
...
> 本节保留 V1.0 单次全文检查图，供已实现版本追溯。V1.1 实施时以第 17.4 节的块级异步图为准；其中 `check_format` 与 `reflect_validation` 分别替换为块级检查和块级反思节点。
...
> 本节保留 V1.0 的检索范围、规则证据与覆盖门禁。V1.1 的运行顺序、结构化范围分发、规则缓存和块级完整性以第 17.2 节为准：规则清单覆盖确认必须在分发前完成，不能按论文章节重复检索同一规则。
...
> 本节描述 V1.0 的全文节点语义。V1.1 将相同的证据门禁和反思路由下沉到每个 `unit_id`，并在块级反思通过后发送阶段结果，详见第 17.5 节。
...
> 下列 V1.0 实体与接口保持兼容。V1.1 需要的块计划、块运行状态、事件序列和 `unit_id` 字段以第 17.7 节的增量定义为准。
...
> 下列 V1.0 展示约定在 V1.1 中扩展为块级阶段结果、稳定顺序和 SSE 重连协议，详见第 17.6 节。
...
| `FormatReviewItem` | 增加 `category`、`aspect`、`evidence_status`、`annotation_json`；保留合成的 `rule_id` 作为旧客户端兼容键 | 保持 `page_numbers` 供旧前端使用。 |
...
3. 复用论文文件访问接口，在格式审查页内嵌 PDF.js；点击问题后使用 `page`、`bbox` 和旋转信息渲染对应页并绘制高亮。
...
## 16. 当前实现同步（2026-07-21）

本节记录 V1.0 独立 `format_review` LangGraph 工作流的已实现能力，论文阅读图、阅读问答智能体、`PaperChunk` 语义检索链路均未改变。第 17 节定义已确认但尚未实施的 V1.1 章节化异步审查方案；两者不应混同。

| 设计项 | 落地实现 |
| --- | --- |
| 受控档案和投稿模式隔离 | `FormatProfile` 保存 `venue_id`、`allowed_submission_modes`、通用规则文档 ID、投稿模式文档映射和版本化规则清单；创建审查请求必须显式传入 `submission_mode`。 |
| 规则清单完整性 | 管理端创建档案时校验 `venue_id`、`format_version`、`submission_mode`、`target_document`、`canonical_rule_id`、`rule_category`、`source_document_id`、`section_path`、`effective_from` 和 `status`；规则范围必须与档案一致。 |
| 审查快照 | `profile_snapshot_json` 冻结规范版本、投稿模式、数据集、两份规则文档、规则清单哈希及模型/提示词配置快照。 |
| 受限检索和覆盖检查 | `retrieve_standard` 仅向冻结的 `dataset_id` 与两份 `document_ids` 发起检索；按 `canonical_rule_id` 统计覆盖，最多补检索 3 次，缺失类别/规则只生成 `unverifiable`。 |
| 综合检查与反思 | `check_format` 通过 `AgentRunner` 调用版本化 `format_check` Prompt；`reflect_validation` 通过 `format_reflect` Prompt 返回枚举路由，证据恢复和检查修复均有结构化上限。 |
| PDF 版面事实 | `PdfTextSpanRecord` 保存 PyMuPDF 的原生 span（字体、字号、flags、颜色、页面尺寸、旋转和 `page_top_left_pt` 坐标）。为遵守“不变更论文阅读模块”的边界，提取在格式审查任务首次运行时惰性执行并按论文版本缓存，不写入 `PaperChunk`。 |
| 证据闭环 | 非合规或合规结论必须同时引用规范原文和带页码/坐标的论文事实；任一证据不完整、扫描页缺少原生样式或规则召回不完整时，确定性降级为 `unverifiable`。 |
| 报告、取消和审计 | 报告保存分类发现、标注坐标、覆盖报告和调用指标；提供取消与 SSE 事件接口；节点 Trace 保存输入/输出摘要哈希、路由及指标。 |
| 前端展示 | `ReviewPage.vue` 不再提供规则勾选，改为论文、格式档案和投稿模式选择；报告按类别展示双证据和 `unverifiable` 原因，并使用 PDF.js 预览定位框。 |
...
其中 `20260721_0004` 扩展投稿模式、综合发现与 `unverifiable` 约束；`20260721_0005` 新增 `pdf_text_spans` 原生样式事实表。`PyMuPDF` 是后端运行依赖，已在 `backend/pyproject.toml` 声明。

### 16.2 网页复现步骤

1. 安装后端依赖并配置 `RAGFLOW_BASE_URL`、`RAGFLOW_API_KEY`、`LLM_BASE_URL`、`LLM_API_KEY` 和 `LLM_MODEL`；执行数据库迁移并启动 API。
2. 以管理员身份创建档案：配置独立 `dataset_id`、共享文档 ID、每个投稿模式专用文档 ID，以及与档案版本匹配的完整规则清单。
3. 上传 PDF，等待论文状态为 `ready`，在“格式审查”页依次选择论文、规范档案和投稿模式后提交。
4. 任务完成后展开“证据详情”；对包含坐标的发现点击“定位原文”，确认 PDF.js 高亮覆盖对应文本区域。规则或版面事实无法完整获取时，应显示“无法可靠判断”，而不是“符合”或“需人工核对”。

## 17. V1.1 章节化异步审查方案（已确认，待实施）

本节优先于前文中“整篇论文一次性综合调用”或“`check_format` 单次完成全文判断”的描述。V1.0 的档案隔离、规则清单完整性、PDF 事实、双证据门禁、取消、追踪和最终结果枚举继续有效；仅将模型判断与反思的执行粒度改为审查块。

### 17.1 目标与边界

1. 不把整篇论文的所有文本跨度和所有规则放入一次 LLM 调用。
2. 不按固定 token 长度切分。审查块由论文结构和版面角色确定，至少包含标题页、摘要、正文语义章节、图表/公式区域、参考文献、附录，以及页数、页边距、匿名信息等跨章节块。
3. 每条启用规则必须具有结构化适用范围和所需论文证据的声明。运行时不能让 LLM 自行决定“某规则是否应该交给某块”；规则适用范围、分发和完整性检查均由可审计的确定性组件完成。
4. 论文从文首到文末按位置编号展示，但块任务无需严格串行执行；运行时采用受限并发，默认并发度为 2 至 3，可由 Harness 配置冻结。
5. 每块必须先经过块级反思校验，才可通过 SSE 作为“已核验阶段发现”展示。前端绝不把 `SectionFormatCheckAgent` 的原始候选结果当作结论展示。
6. 全部块终态后允许调用 `FormatSynthesisAgent`。该智能体仅接收已核验的块摘要、发现、覆盖报告和证据 ID，不重新读取整篇论文或绕过确定性门禁。

### 17.2 结构化规则范围与确定性分发

#### 17.2.1 版本化规则清单契约

`FormatProfile` 的版本化规则清单是运行时的唯一适用性来源，不以 RAGFlow Chunk 的自由文本或可选元数据推断范围。以下已有字段继续必填：`venue_id`、`format_version`、`submission_mode`、`target_document`、`canonical_rule_id`、`rule_category`、`source_document_id`、`section_path`、`effective_from`、`status`。每条处于 `active` 状态的规则还必须具有以下新增字段：

| 字段 | 类型 | 约束与用途 |
| --- | --- | --- |
| `applicable_unit_kinds` | 非空枚举数组 | 仅允许 `front_matter`、`abstract`、`body_section`、`figure_table`、`reference`、`appendix`、`global`；用于把规则分配给相应的审查块类型。 |
| `is_global` | boolean | 为 `true` 的规则只能交给唯一的全篇/跨章节块，不得在普通块重复执行。 |
| `requires_cross_unit` | boolean | 为 `true` 时必须声明允许关联的块类型或页面范围；用于页数、页边距、匿名信息、引用与参考文献一致性等规则。 |
| `applicability_conditions` | 结构化对象 | 至少支持 `requires_object_types`、`requires_section_roles`、`requires_submission_mode`；条件判断必须引用 PDF 结构事实或冻结投稿模式。 |
| `evidence_selector` | 非空枚举数组 | 声明规则判断必需的 PDF 事实类型，例如 `page_geometry`、`font_style`、`caption`、`reference_entry`、`author_identity`。缺少所需事实时不得输出合规或不合规。 |

`FormatProfileUpsertInput`、规则清单 Schema 校验器、管理员档案编辑/导入页面均须校验并展示这些字段。字段必须写入随 `profile_snapshot_json` 冻结的版本化规则清单；不能只写入未经约束的 RAGFlow Chunk metadata。离线规则入库可以使用 LLM 协助初标，但管理员确认后才能启用；运行时不调用 LLM 分配规则。

升级迁移遵循以下策略：已有规则能从源 JSONL 回填的字段自动回填；无法可靠回填的字段保持待补全。缺任一必填范围字段的档案在 V1.1 中标记为 `non_executable`，管理员补齐并通过校验后才允许启用，不影响历史报告的读取。

#### 17.2.2 检索、缓存与分发

`retrieve_standard` 始终按冻结的 `profile_snapshot` 和 `submission_mode` 在共享规则文档与模式专用规则文档中检索完整适用规则集。检索可按 `rule_category` 分批进行，以便控制 RAGFlow 查询与上下文大小；它不按论文章节物理拆分，也不为每个审查块重复检索同一规则。

每个类别的已解析规范证据以 `profile_snapshot_hash + submission_mode + rule_category` 为键存入本次审查的不可变缓存。系统以清单中的 `canonical_rule_id` 校验覆盖：先确认完整规则集，再允许分发；缺失规则先受限补检索，仍缺失时将对应规则记录为 `unverifiable`，绝不静默遗漏或以模型常识补全。全篇规则只检索、装配和检查一次。

在 `extract_pdf_layout` 完成后，`plan_review_units` 只根据稳定的 PDF 结构事实生成块计划；它可与完整规则检索并行执行。两者汇合后，`deterministic_allocate_rules_to_units` 按清单字段和论文事实生成不可变的分发台账：

```json
{
  "unit_id": "u-003",
  "expected_rule_ids": ["heading.level_1", "body.font"],
  "allocated_rule_ids": ["heading.level_1", "body.font"],
  "global_rule_ids": [],
  "not_applicable_rule_ids": [
    {"rule_id": "figure.caption", "condition_evidence_ids": ["pdf.object-index:no-figure"]}
  ],
  "retrieved_rule_ids": ["heading.level_1", "body.font"],
...
1. 每条启用且适用的规则必须至少分配给一个块，或以满足条件的 PDF/模式事实明确列入 `not_applicable_rule_ids`。
2. `is_global=true` 的规则恰好进入一个 `global` 或跨章节块；`requires_cross_unit=true` 的规则必须记录其允许的块/页引用范围。
3. 每个块的完整性以 `expected_rule_ids` 与已检索的 `canonical_rule_id` 比对；缺少任一必需 ID 时，该规则范围为 `unverifiable`，不能把不完整规则集交给 LLM 判断为合规。
4. 分发记录、条件证据、规则证据 ID 和规则清单哈希随审查快照持久化，供恢复、审计和前端解释使用。
...
```json
{
  "review_units": [
    {
      ...
      "applicable_categories": ["heading", "body", "figure", "table"],
      "expected_rule_ids": ["heading.level_1", "body.font"],
      "allocated_rule_ids": ["heading.level_1", "body.font"],
      "status": "pending"
    }
  ...
  "unit_runs": {
    "u-003": {
      ...
      "standard_evidence_ids": ["S12", "S13"],
      "paper_evidence_ids": ["P37", "P38"],
      "candidate_findings": [],
      "validated_findings": [],
      "coverage": {"expected_rule_ids": ["heading.level_1", "body.font"], "retrieved_rule_ids": ["heading.level_1", "body.font"], "missing_rule_ids": []},
      "event_sequence": 4
    }
  },
  "synthesis": {"status": "pending", "deduplicated_finding_ids": []}
}
```

`review_units`、规则分发台账与 `unit_runs` 必须持久化或可从 LangGraph 检查点恢复。每个事实、规则和发现都带 `unit_id`；跨章节块可引用多个页面，但不得读取未声明范围外的全文事实。最终报告保留 `unit_id` 以支持追踪和前端按论文顺序展开。

### 17.4 V1.1 图与智能体

> V1.1 运行策略补充（2026-07-22）
>
> - 规则清单保留完整规范与“PDF 可观测执行集”。只有 `status=active` 且 `observability=pdf_observable` 的规则参与 RAGFlow 覆盖校验、确定性分发和 LLM 审查；`status=disabled` 的规则只在管理员清单中保留，不产生审查块或 `unverifiable` 项。
> - 禁用规则必须记录 `excluded_reason`。首期排除模板/样式文件、LaTeX/Word 源文件、编译和选项、投稿网站流程及内容性要求，因为它们不能仅由 PDF 页面证据可靠溯源。
> - 首期执行集仅包含可按页码、文本、字体或坐标举证的页面版式、摘要、标题层级、图表题注，以及匿名稿中的作者/致谢显性信息。
> - 连续的一级正文按语义合并为最多三个块；无一级标题时按连续页分组。标题/投稿信息、摘要、正文、图表、参考文献、附录和全篇块始终保留在前端计划中；没有适用规则的结构块完成后明确显示“本轮无适用 PDF 规则”，不调用 LLM。
> - 若规则和局部 PDF 事实装配后超过安全上下文预算，系统先按连续页的确定性边界递归细分该块（最多三层）；每个子块继承完整适用规则集，不能截断规则。只有最小页级块仍超限时，才以可审计原因标记 `unverifiable`。

```mermaid
flowchart TD
    S([START]) --> INIT["init_review / 冻结档案"]
    INIT --> PDF["extract_pdf_layout / 一次提取并缓存事实"]
    PDF --> SPLIT["plan_review_units / 仅依据 PDF 结构生成语义块"]
    INIT --> RET["retrieve_standard / 完整规则召回与 canonical_rule_id 覆盖检查"]
    SPLIT --> ALLOCATE["deterministic_allocate_rules_to_units / 规则范围分发与完整性门禁"]
    RET --> ALLOCATE
    ALLOCATE --> DISPATCH["dispatch_units / 受限并发 2-3"]
    DISPATCH --> CHECK["SectionFormatCheckAgent / 块级综合判断"]
    CHECK --> VALIDATE["SectionFormatValidationAgent / 块级反思与双证据门禁"]
    VALIDATE -->|"confirmed 或 unverifiable"| EMIT["emit_unit_result / SSE 已核验阶段结果"]
    VALIDATE -->|"recover / clarify / repair 且剩余一次完整重试"| RETRY["块级恢复、重装配与完整重检"]
    RETRY --> CHECK
    VALIDATE -->|"重试预算耗尽仍无法通过"| UNVER["mark_unit_unverifiable / 持久化终态并 SSE"]
    UNVER --> JOIN["等待全部块终态"]
    EMIT --> JOIN["等待全部块终态"]
    JOIN --> SYNTH["FormatSynthesisAgent / 结构化汇总"]
    SYNTH --> FINAL["deterministic_finalize / 覆盖、去重、结果门禁"]
    FINAL --> REPORT["generate_report / 报告与 PDF 标注"]
    REPORT --> END([END])
```

| 节点/智能体 | 职责 | 不承担的职责 |
| --- | --- | --- |
| `plan_review_units` | 根据章节层级、页面和块角色生成稳定审查块计划 | 不作合规判断；不分配规则；不调用 LLM。 |
| `retrieve_standard` | 检索冻结档案内完整规则证据并校验 `canonical_rule_id` 覆盖 | 不按论文章节进行检索分片；不依赖 LLM 判断召回是否完整。 |
| `deterministic_allocate_rules_to_units` | 以结构化范围字段、规则缓存与 PDF 条件事实为每块计算完整规则集 | 不调用 LLM；不以关键词或模型推断代替范围字段。 |
| `SectionFormatCheckAgent` | 在单块完整适用规则和块级事实内形成候选发现 | 不编造 PDF 数值、坐标或规则；不输出全篇结论。 |
| `SectionFormatValidationAgent` | 反思候选发现的规则适用性、标准证据、论文页码/坐标、建议和冲突 | 不将未经证据支持的候选放行。 |
| `emit_unit_result` | 事务提交已核验阶段发现后发送 SSE | 不暴露原始 Prompt、模型思维链或未校验候选。 |
| `FormatSynthesisAgent` | 合并重复发现、解释块间关联、形成结构化摘要 | 不新增缺少块级证据的发现，不改变确定性结果枚举。 |
| `deterministic_finalize` | 按规则覆盖、证据闭环和全篇规则状态确定最终结果 | 不调用 LLM。 |

### 17.5 块级反思、共享重试预算与结果语义

块级流程固定为“规则/事实装配 -> 候选判断 -> 反思校验 -> 流式展示”。`SectionFormatValidationAgent` 仍保留 `recover_pdf_evidence`、`clarify_standard`、`repair_check`、`confirmed`、`unverifiable` 等枚举路由，但计数器按 `unit_id` 隔离，避免一个失败章节耗尽其他章节的预算。

一个审查块最多经历一次完整重试：首次完整周期与至多一次重试周期，故 `unit_cycle_count <= 2`。完整周期定义为“装配规则/事实 -> `SectionFormatCheckAgent` -> `SectionFormatValidationAgent`”。`recover_pdf_evidence`、`clarify_standard` 与 `repair_check` 共用同一重试预算，不得各自额外获得一次重试：

| 条件 | 确定性处理 |
| --- | --- |
| 首次校验要求恢复、澄清或修复 | 写入 `last_retry_reason`，消耗唯一重试预算，重装配后完整执行第二周期。 |
| 第二周期仍要求恢复、澄清或修复 | 保留已核验发现，将未解决规则范围标为 `unverifiable`，持久化块终态并发送 `unit_unverifiable`。 |
| 规则覆盖缺失、PDF 事实不足或调用失败且无法在预算内恢复 | 以受影响规则范围结束为 `unverifiable`；不得无限重试，也不得丢弃已核验发现。 |

`unit_cycle_count`、`retry_budget_remaining`、`last_retry_reason`、`expected_rule_ids`、`retrieved_rule_ids` 和 `event_sequence` 是检查点与持久化状态的必填字段。AgentRunner 的单次 HTTP/结构化输出机械重试仍受其自身调用策略约束，但不改变上述业务周期上限。

“阶段结果”只表示该块已完成校验而尚未经过全篇汇总；它不是未经反思的草稿，也不是最终全篇合规结论。最终 `compliant`、`non_compliant`、`not_applicable` 与 `unverifiable` 继续遵守原有双证据和规则覆盖门禁。任一块失败、规则缺失或事实不足时，只影响相关块/类别并以 `unverifiable` 显式呈现，不得用模型常识补全。

### 17.6 SSE、前端与取消协议

`GET /api/v1/format-reviews/{format_review_id}/events` 是阶段结果的实时通道。`POST /api/v1/format-reviews` 成功返回 `format_review_id` 后，前端立即建立 SSE 连接；每次块状态或阶段发现先在同一数据库事务中持久化，再将带单调递增 `event_id` 与 `event_sequence` 的事件追加到 Redis/SSE 流。提交失败不得发事件，事件发送失败也不得回滚已持久化状态。

前端以 `format_review_id + unit_id` 建立块状态存储，以 `event_id` 或 `unit_id + event_sequence` 去重；断线时使用 `Last-Event-ID` 或 `after_sequence` 恢复事件。若事件流不可用，前端轮询 `GET /tasks/{id}` 与 `GET /format-reviews/{id}` 读取已持久化状态。界面按 `unit_position` 固定排序，而不是按事件抵达顺序排序；运行、完成和无法判定状态仍应在收到事件时立即更新。

| 事件 | 最小数据 | 前端行为 |
| --- | --- | --- |
| `unit_started` | `unit_id`、位置、标题、页面范围 | 按论文顺序显示该块“审查中”。 |
| `unit_progress` | `unit_id`、阶段、进度 | 显示规则检索、块级判断或证据核验状态。 |
| `unit_validated` | `event_id`、`unit_id`、`event_sequence`、已核验发现、证据 ID、覆盖摘要 | 仅在事务提交和双证据校验后展示可展开的阶段发现与 PDF 定位入口。 |
| `unit_unverifiable` | `unit_id`、类别、原因、缺失证据/规则 | 展示“无法可靠判断”，不展示合规推断。 |
| `unit_failed` | `unit_id`、可重试标记、错误码 | 标记该块失败，其他块继续；汇总时作为受影响范围。 |
| `synthesis_started` / `synthesis_completed` | 汇总状态、去重数量 | 显示“正在汇总”与最终报告就绪。 |

事件载荷不得包含原始 Prompt、模型思维链、未经验证的模型候选、访问令牌或任何密钥配置。取消请求停止尚未开始的块，并在当前模型/工具调用返回后的节点边界停止后续路由；已验证的块结果和 Trace 保留。最终报告只在全部块成功、不可判定、失败或取消均已达到确定终态后生成。

### 17.7 数据、接口与验收增量

1. `FormatProfileUpsertInput`、规则清单表/JSON Schema、导入服务和管理员页面增加第 17.2.1 节字段；后端在创建或启用档案时拒绝缺失、枚举非法、全篇规则多重分配或条件无可用证据选择器的配置。
2. `FormatReview` 增加块计划摘要、规则清单哈希、规则缓存键、并发配置快照、已完成块计数与汇总状态；`FormatReviewItem` 增加 `unit_id`、`unit_position` 和阶段/最终来源标识。新增 `FormatReviewUnit` 时，以 `(format_review_id, unit_id)` 唯一约束保存运行状态、分发台账、重试计数、覆盖报告和事件序列。
3. `GET /format-reviews/{id}` 返回按 `unit_position` 排序的阶段结果、分发/覆盖摘要和最终汇总状态；原有最终 `items` 字段保持兼容。事件端点支持 `Last-Event-ID` / `after_sequence` 恢复。
4. 每次块调用 Trace 记录块 ID、预期/已检索规则 ID、事实 ID、条件证据、选择理由、输入预算、耗时、token、重试和反思路由；汇总调用记录输入发现 ID，禁止记录整篇 PDF 正文。
5. 验收必须覆盖：档案拒绝缺失或非法范围字段；每个启用规则均分配或可证据化地 `not_applicable`；全篇规则仅检查一次；块覆盖以 `expected_rule_ids` 比对已检索 `canonical_rule_id`；块任务受限并发（2 至 3）；每块不超过两次完整周期；块级反思和持久化后才可见；SSE 重连不丢失、不重复；单块失败不阻塞其他块；跨章节规则、取消恢复、最终汇总不新增无证据发现，以及总耗时和首个已核验结果时延。

### 17.8 实施顺序

1. 在不改动论文阅读模块的前提下，先扩展规则清单 Schema、`FormatProfileUpsertInput`、管理员导入/编辑校验和旧档案回填迁移；未补齐范围字段的档案不得进入 V1.1 执行路径。
2. 实现档案级完整规则检索、`canonical_rule_id` 覆盖报告、不可变规则证据缓存和 `deterministic_allocate_rules_to_units`，为每个块持久化分发台账。
3. 定义审查块、块运行状态、两周期共享重试策略、数据库迁移和可恢复 SSE 事件 Schema；保持既有最终报告接口兼容。
4. 将现有全文 `check_format -> reflect_validation` 拆为块级智能体与按块计数的受限回环，实现受限并发、检查点恢复、按位置事件排序和前端阶段结果区域。
5. 接入 `FormatSynthesisAgent` 与确定性最终门禁，完成去重、跨块规则和最终报告。
6. 使用黄金集评测首个已核验结果时延、端到端时延、成本、块级证据完整率和最终结论一致性；验证重连事件不丢失/不重复后，移除全文单次调用路径。
