...
from ..errors import AnswerPersistenceFailed, ModelOutputInvalid, WorkflowCancelled
...
class WorkflowNodes:
    ...
    async def standard_retrieve(self, state: ReviewGraphState) -> dict[str, Any]:
        ...
        if route.effective_route_type == "score" and not command.configuration.standard_version:
            ...
            return {
                "standard_evidences": [],
                "skip_reviews": True,
                "warnings": [*warnings, "PUBLIC_KB_UNAVAILABLE: score standard is not frozen"],
                "error_code": "PUBLIC_KB_UNAVAILABLE",
                "error_message": "评分所需的公共评审标准不可用。",
                "sequence": self._events.sequence,
            }
        ...
        if not standards:
            ...
            return {
                ...
                "error_message": (
                    "评分所需的公共标准不可用。" if route.effective_route_type == "score" else None
                ),
                "sequence": self._events.sequence,
            }
    ...
    async def review_b(self, state: ReviewGraphState) -> dict[str, Any]:
        ...
        understanding = _understanding(state)
        review_a = (
            ReviewOpinions.model_validate(state["review_a"]) if state.get("review_a") else None
        )
    ...
    async def safe_refusal(self, state: ReviewGraphState) -> dict[str, Any]:
        command = _command(state)
        route = _route(state)
        started = await self._begin(state, "refusing", "现有证据不足，正在生成保守说明")
        ...
        await self._trace(command, "safe_refusal", started, "succeeded", error_code=code)
        return {
            "draft_answer": draft.model_dump(mode="json"),
    ...
    async def finalize(self, state: ReviewGraphState) -> dict[str, Any]:
        ...
        answer = AnswerView(
            ...
            score=draft.score,
            review_opinions=[
                *_review_opinions("review_a", state.get("review_a")),
                *_review_opinions("review_b", state.get("review_b")),
            ],
            standards=_standard_references(
                _evidences(state, "standard_evidences"), command.configuration.standard_version
            ),
        ...
        try:
            ...
            await self._trace(command, "finalize", started, "failed", "ANSWER_PERSIST_FAILED")
...
def _review_opinions(reviewer: str, value: dict[str, Any] | None) -> list[ReviewOpinionView]:
    if value is None:
        return []
    reviews = ReviewOpinions.model_validate(value)
    return [
        ReviewOpinionView(
            reviewer=reviewer,
            position=opinion.position,
            summary=(
                "；".join(claim.reasoning_summary for claim in opinion.claims) or opinion.dimension
            ),
            ...
            evidence_ids=list(
                dict.fromkeys(
                    ...
                    for evidence_id in [*claim.paper_evidence_ids, *claim.standard_evidence_ids]
                )
            ),
        )
...
def _standard_references(
    evidences: list[EvidenceItem], standard_version: str | None
) -> list[StandardReference]:
    version = standard_version or "unknown"
    return [
        StandardReference(
            evidence_id=evidence.evidence_id,
            name=str(evidence.metadata.get("name") or evidence.section_title or "公共评审标准"),
            version=str(evidence.metadata.get("version") or version),
        )
