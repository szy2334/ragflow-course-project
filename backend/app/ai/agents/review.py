    ...
    async def run(
        ...
        *,
        ...
        messages = self._prompts.render(
            ...
            paper_evidence_json=as_json(
                [item.model_dump(mode="json") for item in paper_evidences]
            ),
...
def _review_result(
    name: str, reviews: ReviewOpinions, metrics: ModelCallMetrics
) -> AgentResult:
    claims: list[Claim] = []
        ...
        for claim_index, review_claim in enumerate(opinion.claims, start=1):
            ...
            claims.append(
                Claim(
                    ...
                    verdict=(
                        "refuted"
                        if review_claim.review_a_verdict == "unsupported"
                        else "supported"
                    ),
