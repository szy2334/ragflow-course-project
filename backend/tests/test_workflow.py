...
from app.ai.errors import AnswerPersistenceFailed, ModelOutputInvalid, WorkflowCancelled
...
async def test_review_path_runs_a_then_b(command, paper_evidence, standard_evidence):
    ...
    assert {item.reviewer for item in result.answer.review_opinions} == {"review_a", "review_b"}
    assert result.answer.standards[0].evidence_id == "S1"
    assert result.answer.standards[0].version == "standards-v1"


@pytest.mark.asyncio
async def test_review_degrades_when_public_kb_is_empty(command, paper_evidence):
    command = command.model_copy(update={"original_question": "论文的实验设计是否充分？"})
    llm = ScriptedLlm([review_route(), paper_understanding(), review_draft(with_standard=False)])
    retrieval = FakeRetrieval([paper_evidence], [])
...
@pytest.mark.asyncio
async def test_score_refuses_without_frozen_standard_version(command, paper_evidence):
    command = command.model_copy(
        update={
            "original_question": "请为论文实验充分性评分。",
            "configuration": command.configuration.model_copy(update={"standard_version": None}),
        }
    )
    llm = ScriptedLlm([score_route(), paper_understanding()])
    retrieval = FakeRetrieval([paper_evidence])
